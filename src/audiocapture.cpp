...
void RenderArea::paintEvent(QPaintEvent * /* event */) {
    ...
    painter.fillRect(painter.viewport().left() + 10,
                     painter.viewport().top() + 6,
                     pos,
                     painter.viewport().height() - 23,
                     Qt::green);
}
...
bool AudioCapture::Init(const QAudioDeviceInfo &micInfo) {
    ...
    case 8: switch (m_pFormat.sampleType()) {
        case QAudioFormat::UnSignedInt: m_maxAmplitude = 255;
            sample_fmt = AVSampleFormat::AV_SAMPLE_FMT_U8;
            break;
        case QAudioFormat::SignedInt: m_maxAmplitude = 127;
            sample_fmt = AVSampleFormat::AV_SAMPLE_FMT_U8;
            break;
        default: break;
        }
        break;
    case 16: switch (m_pFormat.sampleType()) {
        case QAudioFormat::UnSignedInt: m_maxAmplitude = 65535;
            break;
        case QAudioFormat::SignedInt: m_maxAmplitude = 32767;
    ...
    case 32: switch (m_pFormat.sampleType()) {
        case QAudioFormat::UnSignedInt: m_maxAmplitude = 0xffffffff;
            break;
        case QAudioFormat::SignedInt: m_maxAmplitude = 0x7fffffff;
            sample_fmt = AVSampleFormat::AV_SAMPLE_FMT_S32;
            break;
        case QAudioFormat::Float: m_maxAmplitude = 0x7fffffff; // Kind of
    ...
    if (!micInfo.isFormatSupported(m_pFormat)) {
        QMessageBox::warning(nullptr, tr("Audio Capture Error"),
                             "format is not support");
        return false;
    }

    spdlog::info("default devicdName: {}", micInfo.deviceName().toStdString());
    spdlog::info("sameple: {}", m_pFormat.sampleRate());
    spdlog::info("channel:  {}", m_pFormat.channelCount());
    spdlog::info("fmt: {}", m_pFormat.sampleSize());
    spdlog::info("bytesPerFrame: {}", m_pFormat.bytesPerFrame());

    m_pSwr = new SwrResample();

    //目标和源采样数据格式
    int channel_count = m_pFormat.channelCount();
    int64_t src_ch_layout = (channel_count == 1
                                 ? AV_CH_LAYOUT_MONO
                                 : AV_CH_LAYOUT_STEREO);
    ...
    nb_sample_size = m_pFormat.channelCount() * (m_pFormat.sampleSize() / 8) *
                     nb_sample;
    src_swr_data = new char[nb_sample_size];

    m_pSwr->Init(src_ch_layout, dst_ch_layout, src_rate, dst_rate,
                 src_sample_fmt, dst_sample_fmt, nb_sample);
    ...
    if (m_maxAmplitude) {
        ...
        for (int i = 0; i < numSamples; ++i) {
            for (int j = 0; j < m_pFormat.channelCount(); ++j) {
                ...
                if (m_pFormat.sampleSize() == 8 && m_pFormat.sampleType() ==
                    QAudioFormat::UnSignedInt) {
                    value = *reinterpret_cast<const quint8 *>(ptr);
                } else if (m_pFormat.sampleSize() == 8 && m_pFormat.sampleType()
                           == QAudioFormat::SignedInt) {
                    value = qAbs(*reinterpret_cast<const qint8 *>(ptr));
                } else if (m_pFormat.sampleSize() == 16 && m_pFormat.
                ...
                } else if (m_pFormat.sampleSize() == 32 && m_pFormat.
                           sampleType() == QAudioFormat::UnSignedInt) {
                    if (m_pFormat.byteOrder() == QAudioFormat::LittleEndian)
                    ...
                    value = qAbs(
                        *reinterpret_cast<const float *>(ptr) * 0x7fffffff);
                    // assumes 0-1.0
                }
