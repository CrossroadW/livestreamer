...
public:
    ...
    virtual int InitEncode(int width, int height, int fps, int bitrate,
                           const char *profile) = 0;
    virtual int Encode(unsigned char *src_buf, unsigned char *dst_buf)
    = 0;
    virtual int StopEncode() = 0;

    inline virtual VCodecConfig &GetCodecConfig() {
        return codec_config;
    }

protected:
    void ParseProfileLevelId(const char *profile, char *profilename,
                             uint8_t *levelidc) {
