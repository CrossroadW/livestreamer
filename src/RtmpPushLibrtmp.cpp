...
bool RtmpPushLibrtmp::SendMetadata(const RTMPMetadata &lpMetaData) const {
    char body[1024] = {0};
...
int RtmpPushLibrtmp::SendSpsPps(const uint8_t *extradata,
                                int extextradata_size) {
    int nal_size = isNalTail(extradata, extextradata_size);
    //计算nalu头长度，4个字节或者3个字节
    ...
    for (int i = 0; i < extextradata_size - 4; i++) {
        if (i != 0 && extradata[i] == 0 && extradata[i + 1] == 0 && extradata[
                i + 2] == 0 && extradata[i + 3] == 1) {
